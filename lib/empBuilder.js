...
class Employee {
    ...
    async showEmp(){
      ...
      const stmnt = 'SELECT employees.id, employees.first_name, employees.last_name, CONCAT(manager.first_name, " ", manager.last_name) AS manager, roles.title, roles.salary, department.dept_name FROM employees INNER JOIN roles ON roles.id = employees.role_id INNER JOIN department on department.id = roles.department_id INNER JOIN employees manager on manager.id = employees.manager_id';
